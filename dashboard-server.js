...
// ============================================
// INNER CIRCLE PORTAL ROUTES
// ============================================

const innerCircleRouter = require('express').Router();
const crypto = require('crypto');

// Middleware to check Inner Circle creator auth
function requireInnerCircleAuth(req, res, next) {
  if (!req.session?.innerCircle?.creatorId) {
    return res.redirect('/inner-circle');
  }
  next();
}

// GET /inner-circle - Landing/login page
innerCircleRouter.get('/', (req, res) => {
  if (req.session?.innerCircle?.creatorId) {
    return res.redirect('/inner-circle/dashboard');
  }
  ...
  res.send(`
    ...
    <body>
      <div class="container">
        <h1>Inner Circle</h1>
        <p class="subtitle">Cult Content's Commitment Program</p>
        
        <div class="commitment">
          <h3>Your Commitment:</h3>
          <ul>
            <li>15 videos/month minimum</li>
            <li>Weekly group calls (required)</li>
            <li>50% target collab commission</li>
            <li>25% on ads</li>
          </ul>
        </div>
        
        <a href="/inner-circle/auth/tiktok" class="btn">
          Login with TikTok
        </a>
      </div>
    </body>
...
// GET /inner-circle/dashboard - Main creator dashboard
innerCircleRouter.get('/dashboard', requireInnerCircleAuth, async (req, res) => {
  const creatorId = req.session.innerCircle.creatorId;
  
  // TODO: Fetch creator data from database
  // For now, mock data
  const creator = {
    name: req.session.innerCircle.name || 'Creator',
    username: req.session.innerCircle.username,
    brands: [
      { name: 'Marketily', videosThisMonth: 8, goal: 15, commission: 0.50 },
      { name: 'Trusted Rituals', videosThisMonth: 12, goal: 15, commission: 0.50 }
    ],
    callRecordings: [
      { date: '2026-06-08', title: 'Growth Partners Group Call', url: '#' }
    ]
  };
  ...
  res.send(`
    ...
    <html>
    <head>
      <title>Inner Circle Dashboard</title>
      <meta charset="utf-8">
      <meta name="viewport" content="width=device-width, initial-scale=1">
      <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    <body>
      <div class="header">
        ...
        <p class="subtitle">@${creator.username} • Inner Circle Member</p>
      </div>
        ...
        ${creator.brands.map(brand => `
          <div class="card">
            ...
            <div class="stat">
              ...
              <strong>${brand.videosThisMonth} / ${brand.goal}</strong>
            </div>
            <div class="progress">
              <div class="progress-bar" style="width: ${(brand.videosThisMonth / brand.goal) * 100}%"></div>
            </div>
            <div class="stat">
              <span>Commission</span>
              <strong>${(brand.commission * 100)}%</strong>
            </div>
            <button class="btn" onclick="logVideo('${brand.name}')">Log Video</button>
          </div>
        `).join('')}
      </div>
      ...
      <div class="card">
        ...
        ${creator.callRecordings.map(rec => `
          <div class="recording" onclick="alert('Recording: ${rec.title}')">
            <strong>${rec.title}</strong>
            <p style="color: #718096; font-size: 0.9rem;">${rec.date}</p>
          </div>
        `).join('')}
      </div>
      
      <script>
        function logVideo(brand) {
          if (confirm('Log a new video for ' + brand + '?')) {
            fetch('/inner-circle/video-log', {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
...
// GET /inner-circle/auth/tiktok - TikTok OAuth
innerCircleRouter.get('/auth/tiktok', (req, res) => {
  const csrfState = crypto.randomBytes(16).toString('hex');
  req.session.innerCircle = { csrfState };
  
  // Mock login for now - redirect to callback
  res.redirect('/inner-circle/auth/callback?code=mock&state=' + csrfState);
});

// GET /inner-circle/auth/callback - OAuth callback
innerCircleRouter.get('/auth/callback', async (req, res) => {
  const { code, state } = req.query;
  
  // Mock creator data
  req.session.innerCircle = {
    creatorId: 'mock_creator_' + Date.now(),
    name: 'Test Creator',
    username: 'testcreator',
    accessToken: 'mock_token'
  };
  
  res.redirect('/inner-circle/dashboard');
});

// POST /inner-circle/video-log - Log a video
innerCircleRouter.post('/video-log', requireInnerCircleAuth, async (req, res) => {
  const { brand } = req.body;
  const creatorId = req.session.innerCircle.creatorId;
  
  console.log(`[Inner Circle] Video logged: ${creatorId} -> ${brand}`);
  res.json({ success: true });
});

// GET /inner-circle/recordings - Call recordings library
innerCircleRouter.get('/recordings', requireInnerCircleAuth, async (req, res) => {
  const recordings = [
    { date: '2026-06-08', title: 'Growth Partners Group Call', source: 'Lark', url: '#' }
  ];
  res.json({ recordings });
});

// POST /inner-circle/signup - Commit to program
innerCircleRouter.post('/signup', requireInnerCircleAuth, async (req, res) => {
  const { brands } = req.body;
  const creatorId = req.session.innerCircle.creatorId;
  
  console.log(`[Inner Circle] Signup: ${creatorId} committed to brands:`, brands);
  res.json({ success: true });
});

// GET /inner-circle/brands - List available brands
innerCircleRouter.get('/brands', requireInnerCircleAuth, async (req, res) => {
  const brands = [
    { id: 'marketily', name: 'Marketily', category: 'Health', slotsAvailable: 5 },
    { id: 'trusted_rituals', name: 'Trusted Rituals', category: 'Wellness', slotsAvailable: 3 }
  ];
  res.json({ brands });
});

// Mount Inner Circle routes
app.use('/inner-circle', innerCircleRouter);
