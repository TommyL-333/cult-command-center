...
// requireAuth (staff/CF-Access), requireClientSession (brand portal), and
// requirePortalAdmin (staff portal-admin) now live in middleware/auth.js —
...
require('./routes/inner-circle-covenant')(app, { requireSession: icSqlite && icSqlite.requireSqliteSession, axios, express, getLarkTenantToken });
...
// requireClientSession now lives in middleware/auth.js (see the require at
...
// requirePortalAdmin now lives in middleware/auth.js (see the require at the
// top of this file).
...
// (open-collab-queue is already mounted above, before the auth wall — its own
// requireAdmin guard covers portal-admin session, x-ic-admin-key, and CF Access,
// so a second post-auth mount here was fully shadowed dead weight. Removed.)
