...
async function upsertClient(t, formData, brand) {
  const brandName = formData.brandName || brand?.name;
  const shopId = String(formData.reacherShopId || brand?.shopId || brand?.reacherShopId || '').trim();
  ...
  if (ugc !== undefined) fields['JoinBrands Budget'] = ugc;
...
// --- public entry ---
async function syncOnboardToOpsEngine(formData, brand) {
  try {
    ...
    const client = await upsertClient(t, formData, brand);
    const tasks = await spawnTasks(t, client.recordId, formData.brandName || brand?.name);
...
module.exports = { syncOnboardToOpsEngine, _internals: { upsertClient, spawnTasks, listAll, tenantToken } };
