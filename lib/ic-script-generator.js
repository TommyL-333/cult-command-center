...
function validateScripts(parsed, expectedCount) {
  ...
  // Relaxed count check: a near-miss (e.g. 4 or 6 when 5 were asked) should NOT
  // force the creator to regenerate. Only a genuine failure (zero scripts) throws.
  if (scripts.length < 1) {
    throw new ScriptGenerationError(
      'WRONG_COUNT',
      'Model returned 0 scripts; expected at least 1 (requested ' + expectedCount + ')'
    );
  }
...
async function generateScripts(payload, productContext) {
  ...
  let msg;
  try {
    const anthropic = new Anthropic({ apiKey, timeout: 55000, maxRetries: 1 });
