...
function buildScriptFields(rec) {
  ...
  fields['Credibility'] = str(rec.credibility);
  fields['Problem'] = str(rec.problem);
  fields['Proof Stack'] = str(rec.proofStack);
  fields['CTA'] = str(rec.cta);
  fields['Visual Hook Ideas'] = str(rec.visualHookIdeas);
  fields['Full Script'] = str(rec.fullScript);
  ...
  // ── Script # → NUMBER ──────────────────────────────────────────────────────
  // Accept scriptNumber (1-based) directly, else scriptIndex (0-based → +1).
  let scriptNo;
  ...
  // ── More text fields ───────────────────────────────────────────────────────
  fields['Creator Handle'] = str(
    rec.creatorHandle !== undefined ? rec.creatorHandle : rec.tiktokHandle
  );
  fields['Brand'] = str(rec.brand !== undefined ? rec.brand : rec.brandName);
  fields['Product'] = str(rec.product !== undefined ? rec.product : rec.productName);

  // ── Generated At → NUMERIC EPOCH (milliseconds) ────────────────────────────
  fields['Generated At'] = toEpochMs(rec.generatedAt);
...
function bitableFieldsToScript(record) {
  ...
  return {
    ...
    hook: text(f['Hook']),
    credibility: text(f['Credibility']),
    problem: text(f['Problem']),
    proofStack: text(f['Proof Stack']),
    cta: text(f['CTA']),
    visualHookIdeas: text(f['Visual Hook Ideas']),
    fullScript: text(f['Full Script']),
    funnelStages: arr(f['Funnel Stages']),
    scriptNumber: num(f['Script #']),
    creatorHandle: text(f['Creator Handle']),
    brand: text(f['Brand']),
...
async function fetchScriptsFromBitable(opts) {
  ...
  const matches = (script) => {
    const h = str(script.creatorHandle).replace(/^@/, '').toLowerCase();
    if (handle && h && h === handle) return true;
    // If a Creator Email column exists it would surface via raw fields below.
    return false;
  };
  ...
  try {
    const conditions = [];
    if (handleRaw) {
      conditions.push({ field_name: 'Creator Handle', operator: 'is', value: [handleRaw] });
      const noAt = handleRaw.replace(/^@/, '');
      if (noAt !== handleRaw) {
        conditions.push({ field_name: 'Creator Handle', operator: 'is', value: [noAt] });
      }
    }
    if (email) {
      // Only meaningful if the table has a Creator Email column; harmless OR term.
      conditions.push({ field_name: 'Creator Email', operator: 'is', value: [o.email] });
    }
    const body = {
      page_size: pageSize,
