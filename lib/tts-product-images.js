...
async function _refreshBrandToken(brand) {
  ...
  try {
    ...
    if (rd && rd.code === 0 && rd.data && rd.data.access_token) {
      ...
      const expiresAt = expVal > 9_000_000_000 ? expVal * 1000 : Date.now() + (expVal || 86400) * 1000;
      const newTok = {
        ...t,
...
async function ttsRequest(brand, method, apiPath, params = {}, body = null) {
  ...
  if (token.expires_at && Date.now() > token.expires_at - 120000) {
...
async function fetchCatalog(brand) {
  ...
  try {
    ...
    for (let page = 0; page < 5; page++) {
      ...
      const r = await ttsRequest(brand, 'POST', '/product/202309/products/search', params, { status: 'ACTIVE' });
