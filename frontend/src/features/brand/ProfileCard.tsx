...
import { updateBrandProfile } from '@/lib/brandApi';

/**
 * Mirrors features/creator/ProfileCard.tsx. Email + TikTok handle are
 * required at signup already — shown read-only. Discord username and SMS
 * opt-in are the two optional fields from the spec.
 */
export function ProfileCard({ profile, onUpdated }: { profile: Profile; onUpdated: (p: Profile) => void }) {
  ...
  async function save() {
    ...
    setSaved(false);
    try {
      await updateBrandProfile({ discordUsername, smsOptIn });
